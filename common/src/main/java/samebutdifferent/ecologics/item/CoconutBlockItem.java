...
public class CoconutBlockItem extends BlockItem 
{
    ...
    public InteractionResult useOn(UseOnContext context) {
        ...
        if (state.is(ModBlocks.COCONUT_LEAVES.get()) && context.getClickedFace() == Direction.DOWN && isFreeForPlacement(level, state, pos.below())) {
            level.playSound(context.getPlayer(), pos, SoundEvents.CROP_PLANTED, SoundSource.BLOCKS, 1.0F, 1.0F);
            level.setBlockAndUpdate(pos.below(), ModBlocks.HANGING_COCONUT.get().defaultBlockState().setValue(HangingCoconutBlock.AGE, 2).setValue(HangingCoconutBlock.PERSISTENT, true));
