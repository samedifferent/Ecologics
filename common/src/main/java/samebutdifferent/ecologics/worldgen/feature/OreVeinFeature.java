...
public class OreVeinFeature extends Feature<OreVeinFeatureConfiguration> {
    ...
    public boolean place(FeaturePlaceContext<OreVeinFeatureConfiguration> context) {
        ...
        inVeinShapeBlockRoughnessNoise.SetFrequency(0.1F); // Insanely high freq to cover a lot of space.
        ...
        int veinHeight = context.random().nextIntBetweenInclusive(config.minY().resolveY(worldGenContext), config.maxY().resolveY(worldGenContext));
        ...
        for (int x = -15; x < 16; x++) {
            for (int z = -15; z < 16; z++) {
                for (int y = 0; y < veinHeight; y++) {
                    ...
                    if (cellLocationNoiseSample < -0.66) {
                        continue;
                    }
