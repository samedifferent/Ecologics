...
{
    public FloweringAzaleaLogBlock(BlockBehaviour.Properties properties) {
        super(properties);
    }
    ...
    public static void shearAzalea(Level world, Player player, BlockPos pos, ItemStack stack, InteractionHand hand, Direction direction, BlockState replacementState) {
        ...
        itementity.setDeltaMovement(0.05D * (double) direction.getStepX() + world.getRandom().nextDouble() * 0.02D, 0.05D, 0.05D * (double) direction.getStepZ() + world.getRandom().nextDouble() * 0.02D);
